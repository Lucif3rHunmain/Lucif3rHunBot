...
@bot.on(admin_cmd(pattern="forward ?(.*)"))
...
async def forw(event): 
  ...
  for channel in channels:
    try:
      ...
      try:
        await bot.send_message(logs_id, f"Error in sending at {channel.chat_id}.")
    ...
    if not previous_message.web_preview and previous_message.photo:
      ...
      for channel in channels:
        try:
          ...
          try:
            await bot.send_message(logs_id, f"Error in sending at {chat_id}.")
            await bot.send_message(logs_id, "Error! " + str(error))
            if error == "The message cannot be empty unless a file is provided":
                event.edit("For sending files, upload in Saved Messages and reply .forward to in.")
                return
      ...
      for channel in channels:
        try:
          await bot.send_message(int(channel.chat_id), raw_text, link_preview = False)
          sent_count += 1
          await event.edit(f"Sent : {sent_count}\nError : {error_count}\nTotal : {len(channels)}")
        except Exception as error:
          try:
...
@bot.on(admin_cmd(pattern="addchannel ?(.*)"))
...
@bot.on(admin_cmd(pattern="rmchannel ?(.*)"))
...
@bot.on(admin_cmd(pattern="listchannels"))
...
@bot.on(admin_cmd(pattern="search ?(.*)"))
